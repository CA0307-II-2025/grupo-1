...
El estudio emplea un modelo logit-binomial para estimar la prevalencia \(p_i\) a partir de conteos \(y_i\) sobre exposiciones \(n_i\), con \(\mathrm{logit}(p_i)=\alpha+X_i^{\top}\beta\) y covariables previamente estandarizadas. Las priors \(\alpha,\beta_j\sim\mathcal N(0,5)\) se especifican en la escala log-odds y, al ser muy dispersas, actúan como priors débiles. La inferencia se realiza con No-U-Turn Sampler (NUTS), un algoritmo HMC que adapta automáticamente el tamaño de paso y la longitud efectiva de la trayectoria, expandiéndola hasta detectar una “no-u-turn”; este procedimiento mejora la exploración del posterior al reducir la autocorrelación y evita fijar manualmente el número de pasos de integración, de acuerdo a @HoffmanGelman2014. El resultado son muestras posteriores mezcladas para \(p_i\) y para \((\alpha,\beta)\), a partir de las cuales se obtienen medias e intervalos de credibilidad por provincia y agregados ponderados por \(n_i\) para país.


### Implementación
`prep_data()` filtra la condición (obesidad/sobrepeso), arma \(y\) y \(n\), estandariza covariables (con protección ante DE=0), mapea provincias a índices cuando hace falta y devuelve matrices listas para modelar junto con metadatos.

`fit_model_binomial_logit()` declara las priors anteriores, construye el predictor lineal \(\alpha + X\beta\), aplica la sigmoide para obtener \(p=\mathrm{sigmoid}(\cdot)\) y define la verosimilitud binomial con `pm.Binomial("y_obs", n=n, p=p, observed=y)`.

La inferencia se realiza con No-U-Turn Sampler (NUTS) vía `pm.sample()`, un HMC adaptativo que ajusta automáticamente la longitud de trayectoria usando gradientes del log-posterior para explorar eficientemente. Parámetros típicos: `draws=2000`, `tune=1000`, `target_accept=0.9`, `chains=4`, `cores=4`. Valores más altos de `target_accept` tienden a disminuir divergencias a cambio de más evaluaciones del gradiente. Varias cadenas permiten evaluar la convergencia con \(\hat R\).

`posterior_summary_by_row()` extrae los draws de \(p_i\) y devuelve media posterior e intervalos de credibilidad al 95 %. Estos resúmenes se usan en tablas, mapas y gráficos.

Una utilidad del script combina los draws de \(p_i\) con pesos \(n_i\) para obtener prevalencias agregadas (por provincia o país). Así se propaga la incertidumbre desde distritos a agregados respetando la exposición.

Funciones como `save_posterior_prevalence_figs()` y `save_posterior_prevalence_mosaic()` guardan la distribución posterior de la prevalencia a nivel país (con media e IC95 %) y un mosaico por provincia que facilita comparar niveles e incertidumbre.

### Significado de los parámetros
\(\alpha\): log-odds cuando todas las covariables estandarizadas valen 0 (en su media).  
\(\beta_k\): efecto de +1 DE en la covariable \(k\) sobre el logit; en odds es \(\exp(\beta_k)\).  
\(p_i\): probabilidad de la condición en la fila \(i\); base para medias, IC/HDI y agregados ponderados.
...
### Diagnóstico y tuning
Convergencia: \(\hat R\approx 1\) y tamaños efectivos altos indican cadenas bien mezcladas.  
Divergencias: si aparecen, elevar `target_accept`, revisar la estandarización y/o usar priors un poco más informativos.  
Predicción y calibración: con los draws de \(p_i\) se evalúan métricas fuera del bloque de muestreo (p. ej., MAE, RMSE) y cobertura de IC/HDI frente a observados.
...
Precisión predictiva: obesidad con MAE \(=0.023\) y RMSE \(=0.032\); sobrepeso con MAE \(=0.021\) y RMSE \(=0.030\). Diferencias promedio entre proporciones observadas y estimadas en el rango 2–3 puntos porcentuales.  
...
### Resumen ejecutivo
Es un modelo logit–binomial bayesiano con priors débiles; la inferencia usa NUTS (HMC) con adaptación automática para explorar eficientemente la posterior. El script estandariza covariables, estima el modelo, resume \(p_i\) por fila y agrega provincias/país ponderando por la exposición, además de graficar distribuciones posteriores comparables entre provincias. El resultado es un pipeline reproducible para cuantificar prevalencias y su incertidumbre a distintos niveles territoriales, con diagnósticos y validaciones que respaldan la calidad del ajuste.
