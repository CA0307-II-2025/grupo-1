...
La validación de los modelos binomiales de enlace logit para las condiciones de **obesidad** y **sobrepeso infantil** se realizó con base en tres dimensiones principales: la convergencia del muestreo NUTS, la precisión predictiva y la calibración posterior. En ambos casos se empleó el mismo procedimiento de estimación, con cuatro cadenas y 2 000 iteraciones por cadena, asegurando la comparabilidad de los resultados entre modelos.

#### Convergencia del muestreo

Los diagnósticos de convergencia del algoritmo NUTS evidencian un excelente comportamiento en ambos modelos. En el caso del modelo de obesidad, el estadístico $\hat{R}$ alcanzó un valor máximo de 1.00, con tamaños efectivos mínimos de muestra de 5 764 en la parte central de las distribuciones y 4 841 en las colas. De manera similar, el modelo de sobrepeso obtuvo $\hat{R}=1.00$, con tamaños efectivos de 5 008 y 4 742, respectivamente. Estos resultados indican que las cadenas convergieron de forma adecuada, explorando homogéneamente el espacio posterior y garantizando la estabilidad de las estimaciones.

#### Precisión predictiva

En términos de ajuste, el modelo de obesidad presentó un **error absoluto medio (MAE)** de 0.023 y un **error cuadrático medio (RMSE)** de 0.032, mientras que el modelo de sobrepeso alcanzó un MAE de 0.021 y un RMSE de 0.030. En ambos casos, las diferencias promedio entre las proporciones observadas y las estimadas por el modelo se sitúan en un rango de 2 % a 3 % en unidades proporcionales, lo que denota una buena capacidad para reproducir las prevalencias empíricas.

#### Cobertura predictiva posterior

La evaluación de la calibración mediante la cobertura predictiva posterior mostró que el **77.8 %** de las proporciones observadas de obesidad y el **89.1 %** de las proporciones de sobrepeso se encuentran dentro del intervalo creíble del 95 % generado por los respectivos modelos. Esto indica que ambos modelos reproducen razonablemente la variabilidad observada, aunque el de sobrepeso presenta una calibración más cercana al nivel nominal. El incremento en la cobertura al incorporar la incertidumbre muestral confirma que los intervalos creíbles reflejan adecuadamente la dispersión esperada bajo el proceso binomial. En el caso de la obesidad, la leve subcobertura observada sugiere una concentración algo mayor de las estimaciones alrededor del promedio nacional, mientras que el modelo de sobrepeso muestra una calibración más ajustada a la dispersión real entre distritos.
